...
2. **Налаштування тестових параметрів**
    ...
    - **Нова функція**: Підтримка заздалегідь визначених послідовностей для затримок та стимулів. За замовчуванням увімкнено використання генерованих послідовностей для забезпечення відтворюваності експериментів. Користувач може вимкнути цю опцію через чекбокси "Use pregenerated" та використовувати справжні випадкові значення.
...
3. **Режими тестування**
    ...
    - **CRT1-3 (реакція вибору 1 із 3 сигналів)** та **CRT2-3 (2 із 3 сигналів)**: передбачені, але можуть бути недоступними або ж мають додаткові обмеження у поточній реалізації.
...
4. **Панель налаштування стимулів**
    ...
    - **Кольори (colors)**: прямокутник 5х4 см, з визначеними кольорами (червоний, зелений, жовтий).
...
3. **Задання параметрів тесту**
    ...
    - **Важливо**: Поруч із слайдерами затримки експозиції та кількості стимулів є чекбокси "Use pregenerated" (увімкнені за замовчуванням). Коли чекбокси активні, використовуються заздалегідь визначені послідовності для відтворюваності результатів. Якщо вимкнути чекбоксиsliders стануть активними та будуть використані справжні випадкові значення.

4. **Перехід до вибору типу тесту (SVMR, CRT1-3, CRT2-3)**
...
Проєкт включає автоматизовані тести для забезпечення якості та стабільності коду.

#### Юніт-тести та інтеграційні тести (Vitest)
...
# Згенерувати звіт про покриття коду
...
# Запустити всі перевірки (типи, лінтер, тести, білд)
npm run check
```

#### E2E тести (Playwright)

Проєкт використовує Playwright для наскрізного (E2E) тестування та візуальної регресії.
...
# Запустити лише тести з реальними таймерами
npm run test:e2e:real-timer

# Запустити тести у інтерактивному режимі UI
...
Візуальні тести автоматично створюють скріншоти UI та порівнюють їх з базовими зображеннями.
...
# Переглянути trace для дебагу (після помилки тесту)
npx playwright show-trace test-results/.../trace.zip
```

**Покриття візуальних тестів:**
- Екран вибору типу тесту (SVMR, CRT1-3, CRT2-3)
- Візуальні підказки для користувача (preview стимулів)
- Обидві мови інтерфейсу (EN/UK)
- Перемикання між типами тестів

Скріншоти зберігаються у: `tests/e2e/*-snapshots/`
---

## 5. Методологія збору та обробки результатів

Кожна спроба під час тестування (Trial) класифікується та зберігається з певним результатом (`TrialOutcome`). Це дозволяє не лише вимірювати швидкість реакції, а й аналізувати точність та характер помилок користувача.

### 5.1. Класифікація результатів спроби (TrialOutcome)

Система розрізняє наступні типи результатів:

*   **Success (Успіх)**: Користувач вчасно та правильно зреагував на цільовий стимул. Тільки ці спроби використовуються для розрахунку часових статистичних показників (середнє, мода тощо).
*   **Miss (Пропуск)**: Користувач не зреагував на цільовий стимул протягом часу його експозиції.
*   **FalseAlarm (Хибна тривога)**: Користувач зреагував на дистрактор (стимул, на який не потрібно реагувати).
*   **CorrectRejection (Правильне ігнорування)**: Користувач правильно проігнорував дистрактор.
*   **MixUp (Переплутування)**: Користувач зреагував на цільовий стимул, але використав неправильну дію (наприклад, натиснув ліву клавішу замість правої у тестах вибору).
*   **FalseStart (Передчасний старт)**: Користувач натиснув клавішу під час паузи (очікування стимулу), коли на екрані нічого не було.

### 5.2. Обробка результатів у статистиці

Статистичний модуль (`ReactionTimeStats`) обробляє масив результатів спроб наступним чином:

1.  **Фільтрація за типом**: Для аналізу часу реакції відбираються лише спроби зі статусом **Success**.
2.  **Попереднє чищення (Hard Outliers)**: Вилучаються занадто швидкі або занадто повільні реакції, що виходять за межі фізіологічно можливих (за замовчуванням: <100 мс або >(700/ЧАС ЕКСПОЗИЦІЇ) мс).
3.  **Статистичне чищення (MAD)**: Застосовується метод медіанного абсолютного відхилення (Median Absolute Deviation) для видалення статистичних викидів. Це дозволяє нівелювати вплив випадкових відволікань або випадкових натискань, які пройшли первинний фільтр.
4.  **Розрахунок помилок**: Спроби зі статусами **Miss**, **FalseAlarm**, **FalseStart** та **MixUp** сумуються для розрахунку загальної кількості помилок та відсотка помилкових реакцій.
5.  **Агрегація**: На основі очищених даних Success-спроб будується гістограма розподілу та розраховуються показники за методикою Лоскутової (функціональний рівень, стабільність реакції, функціональні можливості).

---

## 6. Технічна архітектура

Детальний опис внутрішньої логіки екрана тестування, включаючи машину станів, таймери та обробку вводу, можна знайти у окремих технічних документах:
*   [TestScreen Architecture & Logic (English)](docs/TEST_SCREEN.md)
*   [Архітектура та логіка TestScreen (Українська)](docs/TEST_SCREEN_UA.md)
