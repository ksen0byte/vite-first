...
| SVMR expects `DEFAULT` for every stimulus. | `getExpectedAction` | Characterized | `test-response.test.ts: keeps SVMR responses as DEFAULT for every stimulus`; `app-load.spec.ts: records a successful SVMR response with minimum real-clock settings` | Not reviewed. |
