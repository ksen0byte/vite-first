...
| `SpamDetected` can return to test-type selection through the warning screen. | `src/screens/spam-warning-screen.ts`, router registration | Unverified | `tests/e2e/app-load.spec.ts: spam recovery` | Not reviewed. |
| `Finished` retains the per-trial result map. | `toFinished`, `onTestComplete` | Unverified | `test-transitions.test.ts: finished preserves results` | Not reviewed. |
...
| SVMR expects `DEFAULT` for every stimulus. | `getExpectedAction` | Unverified | `test-response.test.ts: svmr expected action` | Not reviewed. |
| CRT1-3 expects `DEFAULT` for red, square, or animal stimuli; all other stimuli expect `NONE`. | `getExpectedAction`, `isRed`, `isSquare`, `isAnimal` | Unverified | `test-response.test.ts: crt1-3 classification` | Not reviewed. |
| CRT2-3 expects `RIGHT` for red, square, or animal; `LEFT` for green, circle, or plant; otherwise `NONE`. | `getExpectedAction`, classification predicates | Unverified | `test-response.test.ts: crt2-3 classification` | Not reviewed. |
| CRT2-3 response outcomes are success for matching action, mix-up for a wrong action on a target, and false alarm on a distractor. | `processTestResponse` | Unverified | `test-response.test.ts: crt2-3 response outcomes` | Not reviewed. |
...
| `isRed`, `isGreen`, and `isYellow` classify their exact color values. | `isRed`, `isGreen`, `isYellow` | Unverified | `stimulus-sequences.test.ts: color predicates` | Not reviewed. |
| `isSquare`, `isCircle`, and `isTriangle` classify their exact shape values. | `isSquare`, `isCircle`, `isTriangle` | Unverified | `stimulus-sequences.test.ts: shape predicates` | Not reviewed. |
| `isAnimal` recognizes the current English and Ukrainian animal sets. | `isAnimal`, `ANIMALS_EN`, `ANIMALS_UA` | Unverified | `stimulus-sequences.test.ts: animal examples and negatives` | Not reviewed. |
| `isPlant` recognizes the current English and Ukrainian plant sets. | `isPlant`, `PLANTS_EN`, `PLANTS_UA` | Unverified | `stimulus-sequences.test.ts: plant examples and negatives` | Not reviewed. |
| `isNonLiving` recognizes the current English and Ukrainian non-living sets. | `isNonLiving`, `NON_LIVING_EN`, `NON_LIVING_UA` | Unverified | `stimulus-sequences.test.ts: non-living examples and negatives` | Not reviewed. |
