...
| SVMR expects `DEFAULT` for every stimulus. | `getExpectedAction` | Characterized | `test-response.test.ts: keeps SVMR responses as DEFAULT for every stimulus`; `real-timer.spec.ts: records a successful SVMR response with real timers` | Not reviewed. |
