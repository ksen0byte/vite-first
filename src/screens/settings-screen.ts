  ...
  return `<main class="flex-grow bg-base-200" id="main"><form id="personal-data-form" class="mx-auto w-full max-w-[1800px] space-y-3 px-4 py-3">
    <section class="card bg-base-100 shadow-sm"><div class="card-body p-4">
      <div class="grid grid-cols-2 gap-2 md:grid-cols-4">
        <label class="block w-full min-w-0"><span class="mb-1 block text-xs font-medium" data-localize="surnameLabel"></span><input class="input input-bordered input-sm w-full" type="text" id="surname-input" inputmode="text" placeholder=" " value="${escapeHtml(appContext.personalData.lastName)}" required minlength="2" maxlength="50" /></label>
        <label class="block w-full min-w-0"><span class="mb-1 block text-xs font-medium" data-localize="nameLabel"></span><input class="input input-bordered input-sm w-full" type="text" id="name-input" inputmode="text" placeholder=" " value="${escapeHtml(appContext.personalData.firstName)}" required minlength="2" maxlength="50" /></label>
        <label class="block w-full min-w-0"><span class="mb-1 block text-xs font-medium" data-localize="ageLabel"></span><input class="input input-bordered input-sm w-full" type="number" id="age-input" inputmode="numeric" placeholder=" " value="${appContext.personalData.age || ''}" min="10" max="99" step="1" required /></label>
        <label class="block w-full min-w-0"><span class="mb-1 block text-xs font-medium" data-localize="genderLabel"></span><select class="select select-bordered select-sm w-full" id="gender-select" required><option value="" disabled ${appContext.personalData.gender ? '' : 'selected'} data-localize="selectGender"></option><option value="male" ${appContext.personalData.gender === 'male' ? 'selected' : ''} data-localize="male"></option><option value="female" ${appContext.personalData.gender === 'female' ? 'selected' : ''} data-localize="female"></option></select></label>
      </div>
    </div></section>

    <section class="card bg-base-100 shadow-sm"><div class="card-body p-4">
      <div class="grid grid-cols-1 gap-3 md:grid-cols-4">
        <label class="block w-full min-w-0"><span class="mb-1 block text-xs font-medium" data-localize="protocolLabel"></span><select id="protocol-select" class="select select-bordered select-sm w-full"><option value="optimal" ${mode === 'optimal' ? 'selected' : ''} data-localize="optimalProtocol"></option><option value="feedback" ${mode === 'feedback' ? 'selected' : ''} data-localize="feedbackProtocol"></option></select></label>
        <label class="block w-full min-w-0"><span class="mb-1 block text-xs font-medium" data-localize="regimeLabel"></span><select id="mode-select" class="select select-bordered select-sm w-full"></select></label>
        <label class="block w-full min-w-0"><span class="mb-1 block text-xs font-medium" data-localize="submodeLabel"></span><select id="test-type-select" class="select select-bordered select-sm w-full"></select></label>
        <label class="block w-full min-w-0"><span class="mb-1 block text-xs font-medium" data-localize="stimulusTypeLabel"></span><select id="stimulus-select" class="select select-bordered select-sm w-full"></select></label>
      </div>
    </div></section>
...
/**
 * Reads a numeric parameter input strictly against its ParameterDefinition.
 * Empty or non-numeric input falls back to the definition default; valid numbers
 * are rounded and clamped to [min, max]. Unlike a `value || fallback` check this
